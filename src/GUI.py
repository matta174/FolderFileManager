...
class FolderFileManager(tk.Frame):
    ...
    def __init__(self, master):
        ...
        self.source_folder_button = Button(
            master, text="Select folder to watch", command=self.select_watch_folder)
        self.source_folder_button.grid(row=4, column=0, padx=10, pady=10)

        self.folder_text = Label(master, textvariable=self.v, padx=10)
        ...
        self.pictures_folder_button = Button(
            master, text="Select folder to place images", command=self.select_picture_folder)
        self.pictures_folder_button.grid(row=7, column=0, padx=10)

        self.v2 = StringVar(value=self.picture_folder)

        self.target_picture_text = Label(master, textvariable=self.v2, padx=10)
        ...
        self.music_folder_button = Button(
            master, text="Select folder to place music files", command=self.select_music_folder)
        self.music_folder_button.grid(row=8, column=0, padx=10)

        self.v3 = StringVar(value=self.music_folder)

        self.target_music_text = Label(master, textvariable=self.v3, padx=10)
        ...
        self.documents_folder_button = Button(
            master, text="Select folder to place document files", command=self.select_documents_folder)
        ...
        self.save_button = Button(
            master, text='Save', command=self.update_configuration)
        self.save_button.grid(row=10, column=1, pady=10)

        self.runbutton = Button(master, text="Run", command=self.run_process)
        self.runbutton.grid(row=11, column=1, pady=10)
    ...
    def configure_gui(self):
        ...
        # img = tk.PhotoImage(file = r"src\\images\\fileshepherd.png")
